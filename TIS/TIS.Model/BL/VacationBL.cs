...
namespace TIS.Model.BL
{
    public class VacationBL
    {
        ...
        public List<Vacation> GetAll()
        {
            try
            {
                List<Vacation> vacations = new XPCollection<Vacation>(uow).OrderBy(v => v.DateFrom).ToList();
        ...
        public List<Vacation> GetByDate(DateTime dateFrom, DateTime dateTo)
        {
            try
            {
                List<Vacation> vacations = new XPQuery<Vacation>(uow).Where(v => v.DateFrom.Date >= dateFrom.Date && v.DateTo.Date <= dateTo.Date).OrderBy(v => v.DateFrom).ToList();
